...
public:
    ...
    bool hasAccessToSee(AbstractUser* user) const override {
        return true; // Admin can see all users (even nullptr)
    }
};
...
void addStudent(string username, string password, int grade) {
    int index = myfind(users, users + user_cnt, [&username](AbstractUser* user) {
...
int main() {
    ...
    while (1) {
        ...
        cout << "password:";
        cin >> password;
        int index = myfind(users, users + user_cnt, [&username, &password](AbstractUser* user) {
            return user->getUsername() == username && user->isPasswordCorrect(password);
        });
        if (index!=-1) {
